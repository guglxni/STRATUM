...
```
                         ...
                         | EpochSettler  CoverageMonitor  |
                         |        ReserveBalancer         |
                         +---------------+----------------+
                                         |
         events / callbacks across chains (no off-chain keepers)
                                         |
   +------------+   +-----------+   +----+-----+   +-----------+   +-----------+
   |   Across   |   |   Brevis  |   | Uniswap  |   | EigenLayer|   |  Stylus   |
   | CPHR cross |   | ZK fee    |   |  v4 hook |   | LVR yield |   | matching  |
   | chain      |   | accounting|   | (core)   |   | + attest  |   | + ML vol  |
   | reserves   |   |           |   | Unichain |   |           |   | (Arbitrum)|
   +------------+   +-----------+   +----------+   +-----------+   +-----------+
        capital        proof         the hook        yield          compute
        layer          layer         + tranches      layer          layer
```
